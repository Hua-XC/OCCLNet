...
class HXCD_contrastLoss(nn.Module):
    ...
    def forward(self, inputs, label,T=10):
        n=label.shape[0]
        inputs = F.normalize(inputs, p=2, dim=1)
        sim = torch.matmul(inputs, inputs.t())
        T=0.1
        sim=torch.exp(sim/T)
        ...
        loss_pos=sim*mask_pos
        loss_neg=sim*mask_neg
        
        loss=-torch.log(loss_pos.sum()/(loss_pos.sum()+loss_neg.sum()))
...
class HXCD_contrastLosscc(nn.Module):

    def __init__(self, batch_size, margin=0.3):
        super(HXCD_contrastLosscc, self).__init__()
        self.margin = margin
        self.ranking_loss = nn.MarginRankingLoss(margin=margin)
    ...
    def forward(self, inputs, label,T=10):
        ...
        centers = []
        for i in range(n):
            centers.append(inputs[label == label[i]].mean(0))
        #array
        ...
        loss=-torch.log(loss_pos.sum()/(loss_pos.sum()+loss_neg.sum()))+loss_center
