...
HXC='HXC2024-06-27 21:12:09-regdb'
...
parser.add_argument('--mode', default='indoor', type=str,
...
if dataset == 'sysu':
    ...
    for trial in range(10):
        test_trial = trial + 1
        model_path = checkpoint_path + 'regdb_mscm_p4_n6_lr_0.1_seed_0_trial_{}_best.t'.format(test_trial)
        if os.path.isfile(model_path):
            print('==> loading checkpoint {}'.format(args.resume))
            checkpoint = torch.load(model_path, map_location={
                                    'cuda:2': 'cuda:0', 'cuda:1': 'cuda:0'})
